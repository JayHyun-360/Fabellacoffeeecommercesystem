...
const DISPLAY_TYPE_LABELS: Record<
  DisplayType,
...
function ProductModal({ product, onSave, onClose }: ProductModalProps) {
  ...
  const addSetItem = () => {
    ...
    setForm({
      ...
      set_items: [
        ...form.set_items,
        {
          product_name: newSetItem.product_name.trim(),
          quantity: parseInt(newSetItem.quantity, 10) || 1,
        },
      ],
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    onSave({
      ...
      promo_price: form.is_promo
        ? form.promo_price
          ? parseInt(form.promo_price, 10)
          : null
        : null,
      ...
      <div className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto border border-gray-200/50">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewSetItem({ ...newSetItem, quantity: e.target.value })
                  }
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
            ...
            >
              <span
                className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${form.available ? "left-6" : "left-0.5"}`}
              />
            </button>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        {[
          ...
          },
          {
            ...
            sub: `${products.filter((p) => !p.available).length} unavailable`,
            gradient: "from-amber-50 to-orange-50",
          },
        ].map((kpi) => (
          <div
            key={kpi.label}
            className={`bg-gradient-to-br ${kpi.gradient} rounded-3xl p-6 border border-gray-200/50 shadow-md hover:shadow-xl transition-all`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl p-6 border border-gray-200/50 shadow-lg">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-sm text-gray-400">
                ₱{totalRevenue.toLocaleString()} session total
              </p>
            </div>
          </div>
            ...
            <BarChart data={chartData} barSize={28}>
              ...
              <Bar dataKey="revenue" radius={[6, 6, 0, 0]}>
                {chartData.map((entry, i) => (
                  <Cell
                    key={`bar-cell-${entry.day}-${i}`}
                    fill={entry.isToday ? "#000" : "#e5e7eb"}
                  />
                ))}
              </Bar>
            </BarChart>
          ...
          <p className="text-xs text-gray-400 text-center mt-2">
            Revenue reflects active session orders
          </p>
        </div>
          ...
          {["all", "coffee", "food", "pastries", "beverages"].map((cat) => (
            <button
              ...
              className={`px-4 py-2.5 rounded-full text-sm border transition-all ${
                catFilter === cat
                  ? "bg-gradient-to-br from-gray-900 to-black text-white border-black shadow-lg scale-105"
          ...
          >
            <div className="aspect-video bg-gray-100 relative overflow-hidden">
              ...
              {!product.available && (
                <div className="absolute inset-0 bg-white/60 flex items-center justify-center">
                  <span className="bg-white text-xs px-2 py-1 rounded-full border border-gray-200 text-gray-500">
                    Unavailable
                  </span>
                </div>
              )}
            ...
            <div className="p-4">
              <div className="flex items-start justify-between gap-2 mb-1">
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${CATEGORY_COLORS[product.category]}`}
                >
              ...
              <div className="flex items-center gap-2 mb-3">
                <p
                  className={`text-sm ${product.promo_price ? "line-through text-gray-400" : ""}`}
                >
                  ₱{product.price}
                </p>
                {product.promo_price && (
...
function TransactionsSection() {
  ...
  const typeBreakdown = ["dine-in", "takeout", "delivery", "pickup"].map(
    (type) => ({
      type,
      count: orders.filter(
        (o) => o.deliveryType === type && o.status !== "cancelled",
      ).length,
      revenue: orders
        .filter((o) => o.deliveryType === type && o.status !== "cancelled")
        .reduce((s, o) => s + o.total, 0),
    }),
  );
  ...
  const handleRecordReport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        "Failed to record report. Make sure you ran the SQL script to create the sales_records table.",
      );
    }
  };

  const handleDeleteReport = async (id: string) => {
    if (!confirm("Are you sure you want to delete this recorded report?"))
      return;
    try {
      const supabase = createClient();
      const { error } = await (supabase.from("sales_records") as any)
  ...
  const handleClearActiveTransactions = async () => {
    if (
      !confirm(
        "⚠️ DANGER: This will permanently delete ALL active orders and transactions from BOTH your screen and your database, and reset the order queue sequence to #1! This action cannot be undone.\n\nAre you sure you want to clear active transactions?",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      alert(
        "Active transaction history successfully wiped and queue counter reset to #1!",
      );
      if (refreshOrders) {
        await refreshOrders();
      }
      ...
      alert(
        "Failed to clear transactions. Please make sure the 'clear_active_orders' RPC is configured in Supabase.",
      );
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-gray-400">
                {reports.length} recorded report
                {reports.length !== 1 ? "s" : ""} saved in archive
              </p>
            </div>
          </div>
          <ChevronDown
            className={`w-5 h-5 text-gray-400 transition-transform ${showLedger ? "rotate-180" : ""}`}
          />
        </button>
          ...
          <div className="p-6 space-y-4">
            {reports.length === 0 ? (
              <div className="py-8 text-center text-gray-400 text-sm">
                No archived ledgers found. Record your first report using the
              ...
              <div className="grid gap-3 max-h-[400px] overflow-y-auto pr-1">
                {reports.map((report) => {
                  ...
                  return (
                    <div
                      key={report.id}
                      className="border border-gray-100 rounded-2xl p-4 bg-gray-50 hover:bg-white hover:shadow-md transition-all"
                    >
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div>
                          <p className="font-semibold text-gray-900">
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-100 space-y-2">
                          ...
                          <div className="max-h-[200px] overflow-y-auto space-y-1.5 pr-1">
                            {snapOrders.map((ord: any, idx: number) => (
                              <div
                                key={idx}
                                className="flex justify-between items-center text-sm py-1 border-b border-gray-100/50 last:border-0"
                              >
                                <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            sub: `${orders.filter((o) => o.paymentMethod === "gcash" && o.status !== "cancelled").length} txns`,
          ...
          },
          {
            ...
            sub: `${orders.filter((o) => o.paymentMethod === "cod" && o.status !== "cancelled").length} txns`,
          ...
          <div
            key={s.label}
            className={`bg-gradient-to-br ${s.gradient} border ${s.border} rounded-3xl p-5 shadow-sm hover:shadow-md transition-all`}
          >
          ...
          {typeBreakdown.map(({ type, count, revenue }) => {
            ...
            return (
              <div key={type} className={`${cfg.bg} rounded-2xl p-4`}>
                <p
                  className={`text-xs ${cfg.text} uppercase tracking-wider mb-2`}
                >
                  {cfg.label}
                </p>
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          ...
          filtered.map((o) => {
            ...
            const typeCfg = ORDER_TYPE_CFG[o.deliveryType] ?? {
              label: o.deliveryType,
              bg: "bg-gray-50",
              text: "text-gray-600",
            };
            const paymentCfg = PAYMENT_CFG[o.paymentMethod] ?? {
            ...
            return (
              ...
              >
                <div
                  ...
                  onClick={() =>
                    setExpandedOrder(isExpanded ? null : o.id || o.date)
                  }
                >
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span
                      className={`text-xs px-2.5 py-1 rounded-full hidden sm:inline-flex ${typeCfg.bg} ${typeCfg.text}`}
                    >
                      {typeCfg.label}
                    </span>
                    <div
                      className={`items-center gap-1 text-xs ${paymentCfg.color} hidden sm:flex`}
                    >
                      {paymentCfg.icon}
                      {paymentCfg.label}
                    </div>
                    ...
                    <span
                      className={`text-xs px-2.5 py-1 rounded-full ${statusCfg.badge}`}
                    >
                      {statusCfg.label}
                    </span>
                    <ChevronDown
                      ...
                      <div>
                        ...
                        <div className="space-y-2.5 text-sm bg-white rounded-2xl p-4 border border-gray-100">
                          <div className="flex justify-between items-center">
                            ...
                            <span
                              className={`text-xs px-2.5 py-1 rounded-full ${typeCfg.bg} ${typeCfg.text}`}
                            >
                              {typeCfg.label}
                            </span>
                          </div>
                        ...
                        {(o.status === "pending" ||
                          ...
                          <div className="flex gap-2 mt-3">
                            {o.status === "pending" && (
                              <button
                                onClick={() =>
                                  o.id && updateOrderStatus(o.id, "cancelled")
                                }
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {(
          ...
          return (
            <div
              key={s}
              className="bg-white border border-gray-200/50 rounded-3xl p-5 shadow-md hover:shadow-lg transition-all"
            >
              <span
                className={`text-xs px-2.5 py-1 rounded-full ${STATUS_CONFIG[s].badge}`}
              >
            ...
            <form onSubmit={handleRecordReport} className="space-y-4">
              ...
              {reports.length > 0 && (
                <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-100 rounded-2xl text-xs text-amber-800">
                  ...
                  >
                    Overwrite the latest archived report (
                    <strong>{reports[0].report_name}</strong>) instead of
                    creating a new one.
                  </label>
                </div>
              )}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          ...
          <div
            key={s.label}
            className={`bg-gradient-to-br ${s.gradient} rounded-2xl p-5 border border-gray-200/50 shadow-sm`}
          >
...
function SlideEditor({
  ...
  index: number;
  canDelete: boolean;
  onUpdate: (u: Partial<HeroSlide>) => void;
  onDelete: () => void;
}) {
      ...
      >
        ...
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <p className="text-sm tracking-tight font-semibold text-gray-950">
                FABELLA COFFEE
              </p>
